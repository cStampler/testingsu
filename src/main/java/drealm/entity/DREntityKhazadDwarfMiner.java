...
public class DREntityKhazadDwarfMiner extends DREntityKhazadDwarf implements LOTRTradeable {
	...
	public String getSpeechBank(EntityPlayer entityplayer) {
		if (isFriendly(entityplayer)) {
			if (canTradeWith(entityplayer)) {
				return "khazadDwarf/miner/friendly";
			}
			return "khazadDwarf/miner/neutral";
		}
	...
	public void onPlayerTrade(EntityPlayer entityplayer, LOTRTradeEntries.TradeType type, ItemStack itemstack) {
		LOTRLevelData.getData(entityplayer).addAchievement(DRAchievement.trade_red_dwarf_miner);
	}

	@Override
	public IEntityLivingData onSpawnWithEgg(IEntityLivingData data) {
		data = super.onSpawnWithEgg(data);
		npcItemsInv.setMeleeWeapon(new ItemStack(DRRegistry.pickaxe_khazad_dwarven));
